...
(function(){
  ...
  const I18N = {
    en: {
      ...
      hint_words: 'Tap a word to view details. Delete is hard delete.',
      hint_quotes: 'Tap an item to view.',
      ...
      confirm_delete_words: (n)=>`Delete ${n} words?`,
      confirm_delete_quotes: (n)=>`Soft-delete ${n} quotes?`,
      confirm_delete_one_word: 'Delete this word?',
    ...
    zh: {
      ...
      ph_tags: '\u6dfb\u52a0\u6807\u7b7e\uff08\u9017\u53f7\u5206\u9694\uff09',
      hint_words: '\u70b9\u51fb\u5355\u8bcd\u67e5\u770b\u8be6\u60c5\uff1b\u5220\u9664\u4e3a\u786c\u5220\u9664\u3002',
      hint_quotes: '\u70b9\u51fb\u6761\u76ee\u67e5\u770b\u8be6\u60c5\u3002',
      ...
      confirm_delete_words: (n)=>`\u5220\u9664 ${n} \u4e2a\u5355\u8bcd\uff1f`,
      confirm_delete_quotes: (n)=>`\u8f6f\u5220\u9664 ${n} \u6761\u91d1\u53e5\uff1f`,
      confirm_delete_one_word: '\u5220\u9664\u8fd9\u4e2a\u5355\u8bcd\uff1f',
  ...
  const I18N_OVERRIDE = {
    en: {
      ...
      hint_quotes: 'Tap an item to view details.',
      rv_words: '\ud83d\udcd8 Words',
      rv_quotes: '\u2728 Quotes',
      rv_mode_both: 'CN + EN',
      rv_mode_cn: 'CN only',
  ...
  function applyI18n(){
    try{
      ...
      document.title = `${t('brand_title')} 璺?HORD`;
    }catch(_){}
    ...
    $('w-sort-dir') && ($('w-sort-dir').textContent = wordsSortDir === 'asc' ? '閳? : '閳?);
    $('q-sort-dir') && ($('q-sort-dir').textContent = quotesSortDir === 'asc' ? '閳? : '閳?);
    const wSort = $('w-sort');
    if(wSort){
    ...
    $('rv-delete-word') && ($('rv-delete-word').textContent = (lang === 'zh' ? '鍒犻櫎褰撳墠璇? : 'Delete Word'));
    ...
    $('dlg-q-cancel-edit') && ($('dlg-q-cancel-edit').textContent = (lang === 'zh' ? '\u53d6\u6d88\u7f16\u8f91' : 'Cancel Edit'));
    ...
    $('w-hint') && ($('w-hint').textContent = (lang === 'zh' ? '鐐瑰嚮鍗曡瘝鏌ョ湅璇︽儏锛涘垹闄や负纭垹闄ゃ€? : 'Tap a word for details; delete is hard delete.'));
    $('q-hint') && ($('q-hint').textContent = (lang === 'zh' ? '鐐瑰嚮鍙充晶瀵煎嚭鍥剧墖鍙洿鎺ュ鍑哄崟鏉°€? : 'Use the right-side export button for single quote export.'));
    ...
    if(dlgActions && !$('dlg-q-preview-canvas')){
      ...
      box.innerHTML = `
        <div class="row small muted quotePreviewCtrl">
          ...
          <label for="dlg-q-exp-font-adjust" class="small muted">鐎涙褰?/label>
  ...
  function initQuoteExportControls(){
    ...
    const templateMap = lang === 'zh'
      ? {
          hordSignature: 'HORD 閸濅胶澧?,
          editorial: 'Editorial 閺夊倸绻?,
          gradientSoft: 'Gradient 濞撴劕褰?,
          nightCircuit: 'Night 婢舵粓妫?,
        }
      : {
  ...
  function isIOSLike(){
    ...
    return /iPad|iPhone|iPod/i.test(ua) || (p === 'MacIntel' && (navigator.maxTouchPoints || 0) > 1);
  }
  ...
  function updateSortStatePills(){
    ...
    if(w){
      w.textContent = lang === 'zh'
        ? `閹烘帒绨敍?{sortFieldLabel('words', wordsSortField)} ${wordsSortDir === 'asc' ? '閳? : '閳?}`
        : `Sort: ${sortFieldLabel('words', wordsSortField)} ${wordsSortDir === 'asc' ? '閳? : '閳?}`;
    }
    const q = $('q-sort-state');
    if(q){
      q.textContent = lang === 'zh'
        ? `閹烘帒绨敍?{sortFieldLabel('quotes', quotesSortField)} ${quotesSortDir === 'asc' ? '閳? : '閳?}`
        : `Sort: ${sortFieldLabel('quotes', quotesSortField)} ${quotesSortDir === 'asc' ? '閳? : '閳?}`;
    }
  }
  ...
  function buildReviewQueue(asset, desiredLimit){
    ...
    const k = Math.min(Math.max(1, lim), 100, dueIds.length);
  ...
  async function main(){
    ...
    function renderBackups(){
      ...
      for(let i=0;i<backups.length;i++){
        ...
        opt.value = String(i);
        opt.textContent = `${fmtTime(b.ts)}${tag} 璺?w=${words} q=${quotes}`;
        sel.appendChild(opt);
      }
    ...
    $('btn-theme')?.addEventListener('click', ()=>{
      ...
      applyTheme();
      toast('toast-home', lang === 'zh' ? `娑撳顣介敍?{themeModeLabel(themeMode)}` : `Theme: ${themeModeLabel(themeMode)}`);
    });
    ...
    async function exportSelectedQuoteImages(listOverride){
      ...
      try{
        ...
        const iosHint = isIOSLike()
          ? (lang === 'zh' ? '閿涘潟Phone 鐠囧嘲婀化鑽ょ埠閸掑棔闊╅幋鏍ㄦ煀瀵偓妞ょ敻娼版稉顓濈箽鐎涙ê娴橀悧鍥风礆' : ' (On iPhone, save via Share Sheet or long-press in opened tab).')
          : '';
        toast('toast-quotes', lang === 'zh'
          ? `\u5bfc\u51fa\u5b8c\u6210\uff1a\u6210\u529f ${successCount}\uff0c\u5931\u8d25 ${quoteExportRuntime.failed}\uff0c\u91cd\u8bd5 ${quoteExportRuntime.retried}\u3002${iosHint}`
          : `Export done: success ${successCount}, failed ${quoteExportRuntime.failed}, retried ${quoteExportRuntime.retried}.${iosHint}`);
      }catch(e){
        toast('toast-quotes', lang === 'zh'
  ...
  function findQuoteRecord(asset0, id){
    ...
    function renderWordDialogView(rec){
      ...
      if(vCn) vCn.textContent = `CN: ${cn || (lang === 'zh' ? '暂无释义' : 'N/A')}`;
      if(vEn) vEn.textContent = `EN: ${en || (lang === 'zh' ? '暂无释义' : 'N/A')}`;
    ...
    function splitEnglishMeaningInput(raw){
      return String(raw || '')
        .split(/\s*\|\s*|\s*;\s*|\s*\n\s*/)
        .map(x=>String(x || '').trim())
        .filter(Boolean)
    ...
    $('dlg-w-save')?.addEventListener('click', async ()=>{
      ...
      const mr = await commitMutation(asset, ()=>{
        const rec = findWordRecord(asset, dlgWordId);
        if(!rec) return false;
        const meaning = String($('dlg-w-meaning')?.value || '').trim();
        const englishMeaning = splitEnglishMeaningInput($('dlg-w-english')?.value || '');
        const note = String($('dlg-w-note')?.value || '').trim();
    ...
    function renderQuoteDialogView(rec){
      ...
      if(vTxt){
        vTxt.textContent = txt || (lang === 'zh' ? '锛堟殏鏃犲師鏂囷級' : '(no quote text)');
    ...
    function setQuoteDialogEditMode(on){
      ...
      if(tr) tr.setAttribute('inputmode', im);
      if(n) n.setAttribute('inputmode', im);
    ...
    async function exportCurrentDialogQuoteImage(){
      const exporter = globalThis.QuoteCardExporter;
      if(!exporter || typeof exporter.exportPng !== 'function'){
        toast('toast-dlg-quote', lang === 'zh' ? '鐎电厧鍤Ο鈥虫健閺堫亜濮炴潪鏂ょ礉鐠囧嘲鍩涢弬鏉挎倵闁插秷鐦妴? : 'Exporter not loaded. Refresh and try again.');
        return false;
      }
      if(!asset || !dlgQuoteId) return false;
      const rec = findQuoteRecord(asset, dlgQuoteId);
      if(!rec || rec.isDeleted === true){
        toast('toast-dlg-quote', lang === 'zh' ? '鐠囥儵鍣鹃崣銉ュ嚒娑撳秴鐡ㄩ崷銊ｂ偓? : 'Quote no longer exists.');
        return false;
      }
      const sentence = buildCurrentQuoteExportSentence(rec);
      const baseSettings = exporter.normalizeSettings
      ...
      try{
        ...
        toast('toast-dlg-quote', lang === 'zh' ? '瀹歌尪袝閸欐垵顕遍崙鎭掆偓? : 'Export started.');
        return true;
      }catch(e){
        toast('toast-dlg-quote', lang === 'zh'
          ? `鐎电厧鍤径杈Е閿?{String(e && e.message || e || 'unknown')}`
          : `Export failed: ${String(e && e.message || e || 'unknown')}`);
        return false;
      }
    }
    ...
    $('dlg-q-edit')?.addEventListener('click', ()=>{
    ...
    $('dlg-q-save')?.addEventListener('click', async ()=>{
      toast('toast-dlg-quote','');
      if(!asset || !dlgQuoteId || !dlgQuoteEditing) return;
    ...
    $('dlg-q-del')?.addEventListener('click', async ()=>{
      ...
      updateHomeUI(asset);
      renderQuotes();
      toast('toast-quotes', lang === 'zh' ? '\u5df2\u5220\u9664\u3002' : 'Deleted.');
    });
    ...
    function renderReview(){
      ...
      const qPhUs = $('q-phon-us');
      const qPhUk = $('q-phon-uk');
      const phUs = getWordPhonetic(rec, 'us');
      const phUk = getWordPhonetic(rec, 'uk');
      if(qPhUs) qPhUs.textContent = `US: ${phUs || '-'}`;
      if(qPhUk) qPhUk.textContent = `UK: ${phUk || '-'}`;
      const cn = String(getWordMeaning(rec) || '').trim();
      const en = String(getWordEnglishMeaning(rec) || '').trim();
      let lines = '';
    ...
    $('q-play-us')?.addEventListener('click', async ()=>{
      toast('toast-review','');
      if(!asset || !queue.length || qIdx >= queue.length) return;
      const rec = findWordRecord(asset, queue[qIdx]);
      if(!rec) return;
      const ok = await playWordPronounce(rec, 'us');
      if(!ok) toast('toast-review', lang === 'zh' ? '閺冪姴褰查悽銊ュ絺闂婄偨鈧? : 'No pronunciation available.');
    });
    ...
    $('rv-delete-word')?.addEventListener('click', async ()=>{
      ...
      if(!mr.ok){
        toast('toast-review', lang === 'zh' ? `閸掔娀娅庢径杈Е閿?{mr.error || 'unknown'}` : `Delete failed: ${mr.error || 'unknown'}`);
        return;
      }
      ...
      renderWords();
      renderReview();
      toast('toast-review', lang === 'zh' ? '瀹告彃鍨归梽銈嗘拱鐠囧秲鈧? : 'Deleted.');
    });
    const closeReviewMoreMenu = ()=>{
      const menu = $('rv-more-menu');
      if(menu) menu.dataset.open = '0';
    };
    $('rv-more-toggle')?.addEventListener('click', (e)=>{
      e.preventDefault();
      e.stopPropagation();
      const menu = $('rv-more-menu');
      if(!menu) return;
    ...
    $('rv-edit-word')?.addEventListener('click', (e)=>{
      e.preventDefault();
      e.stopPropagation();
      if(!asset || !queue.length || qIdx >= queue.length) return;
      const id = String(queue[qIdx] || '').trim().toLowerCase();
      closeReviewMoreMenu();
      if(!id) return;
      openWordDialog(id);
    });
    $('rv-delete-word')?.addEventListener('click', ()=> closeReviewMoreMenu());
    document.addEventListener('mousedown', (e)=>{
      const menu = $('rv-more-menu');
      const toggler = $('rv-more-toggle');
      if(!menu || menu.dataset.open !== '1') return;
      const t = e?.target;
      if(menu.contains(t) || (toggler && toggler.contains(t))) return;
      closeReviewMoreMenu();
    }, true);
    ...
    if(wSortDir){
      wSortDir.textContent = wordsSortDir === 'asc' ? '閳? : '閳?;
    ...
    for(const expRatio of Array.from(document.querySelectorAll('select[data-exp-key="ratio"]'))){
      expRatio.title = (lang === 'zh' ? '鐎电厧鍤В鏂剧伐' : 'Export ratio');
    }
    for(const expAdj of Array.from(document.querySelectorAll('input[data-exp-key="fontAdjust"]'))){
      expAdj.title = (lang === 'zh' ? '鐎涙褰垮顔跨殶' : 'Font size adjust');
    }
    const qSortDir = $('q-sort-dir');
    if(qSortDir){
      qSortDir.textContent = quotesSortDir === 'asc' ? '閳? : '閳?;
