...
(function(){
  ...
  const REVIEW_LIMIT_MIGRATION_KEY = 'hord_mobile_review_limit_migrated_v2';
  ...
  function getReviewSessionLimit(){
    try{
      // Legacy cleanup: old builds may have pinned default to 50.
      const migrated = String(localStorage.getItem(REVIEW_LIMIT_MIGRATION_KEY) || '') === '1';
      if(!migrated){
        const legacyRaw = Number(localStorage.getItem(REVIEW_LIMIT_KEY) || 0);
        if(legacyRaw === 50){
          localStorage.setItem(REVIEW_LIMIT_KEY, '20');
          localStorage.removeItem(REVIEW_LIMIT_USER_SET_KEY);
        }
        localStorage.setItem(REVIEW_LIMIT_MIGRATION_KEY, '1');
      }
  ...
  function setVersionTag(){
    ...
    // Mobile UI no longer displays version badge by design.
      ...
      box.innerHTML = `
        <div class="row small muted quotePreviewCtrl">
          ...
          <select id="dlg-q-exp-ratio" data-exp-key="ratio" style="min-width:98px;">
            ...
            <option value="iphone">iPhone 壁纸</option>
          </select>
